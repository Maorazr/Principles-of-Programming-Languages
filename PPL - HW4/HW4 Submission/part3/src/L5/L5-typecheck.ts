...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         isDefineTypeExp, isTypeCaseExp, DefineTypeExp, TypeCaseExp, CaseExp, isAtomicExp, VarDecl } from "./L5-ast";
...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         ...
         isRecord, ProcTExp, makeUserDefinedNameTExp, Field, makeAnyTExp, isAnyTExp, isUserDefinedNameTExp, makeLitTExp } from "./TExp";
...
import {isSymbolSExp, SExpValue} from "./L5-value";

// returns all user defined types in the program
// L51
...
// returns all user defined things in the program
// L51
...
export const initTEnv = (p: Program): TEnv => {
    ...
    const globalDefinitions = getDefinitions(p);
    const allRecords = getRecords(p);

    // add makers to TEnv
    const maker_procs = map((record : Record) => makeProcTExp(
                            map((field: Field) => field.te, record.fields), record), allRecords);
    const maker_names = map((record : Record) => "make-".concat((record.typeName)), allRecords);
    let myTEnv = makeExtendTEnv(maker_names, maker_procs, makeEmptyTEnv());

    // add preds to TEnv 
    const record_pred_procs = map((record : Record) => makeProcTExp([makeAnyTExp()], makeBoolTExp()), allRecords);
    const ud_pred_procs = map((ud : UserDefinedTExp) => makeProcTExp([makeAnyTExp()], makeBoolTExp()), userDefined);
    const record_pred_names = map((record : Record) => record.typeName.concat("?"), allRecords);
    const ud_pred_names = map((ud : UserDefinedTExp) => ud.typeName.concat("?"), userDefined);
    myTEnv = makeExtendTEnv(record_pred_names, record_pred_procs, myTEnv); // add record preds
    myTEnv = makeExtendTEnv(ud_pred_names, ud_pred_procs, myTEnv); // add ud preds

    // add global def
    const glob_names = map((x : DefineExp) => x.var.var, globalDefinitions); // get names
    const glob_types = map((x : DefineExp) => x.var.texp, globalDefinitions); 
    myTEnv = makeExtendTEnv(glob_names, glob_types, myTEnv);
    
    return myTEnv;
}

export const getAllRec = (rec:Record, records:Record[]) =>
    filter ((currRec:Record) => currRec.typeName ===  rec.typeName, records);

export const consistentFields = (records: Record[] , first:Record) =>
    reduce((acc , curr)=> (acc && equals(curr.fields ,first.fields)),true, records);

export const isRecursive = (udType: UserDefinedTExp) =>
    udType.records.some((rec:Record) => rec.fields.some((field:Field) => equals(field.te, makeUserDefinedNameTExp(udType.typeName))))

export const hasBaseCase = (udType: UserDefinedTExp) =>
    udType.records.some((rec:Record) => rec.fields.every((field:Field) => !equals(field.te, makeUserDefinedNameTExp(udType.typeName))))

export const isGoodUDType = (udType: UserDefinedTExp) =>
    !isRecursive(udType) || (isRecursive(udType) && hasBaseCase(udType));
...
export const checkUserDefinedTypes = (p: Program): Result <true> => {
    const userDefined = getTypeDefinitions(p);
    const rec = getRecords(p);
    const globallyConsistent = reduce ( (acc: boolean ,curr: Record) => curr && consistentFields( getAllRec(curr, rec), curr), true, rec);
    const possiblyRecursive = reduce ((acc: boolean, curr: UserDefinedTExp) =>  acc && isGoodUDType(curr),true ,userDefined);

    return (globallyConsistent && possiblyRecursive) ? makeOk(true) : makeFailure("not globally consistent or bad recursive");
}
...
const checkTypeCase = (tc: TypeCaseExp, p: Program): Result<true> => 
    ...
    // (in any order)
    oneClauseForEachRecord(tc.cases) && checkVarNum(tc.cases, p) ? makeOk(true) :
    makeFailure("There is a record with more than one clause");

const oneClauseForEachRecord = (cases : CaseExp[]) : boolean => {
    const noDups = new Set(cases);
    return cases.length == noDups.size;
}

const checkVarNum = (cases: CaseExp[], p: Program): boolean => {
    if (cases.length === 0) 
        return true;
    else {
        const numOfVars = first(cases).varDecls.length;
        const numOfFields = bind(getRecordByName(first(cases).typeName, p), (r : Record) => makeOk(r.fields.length));
        if (isOk(numOfFields)) {
            if (numOfFields.value != numOfVars) {
                return false;
            }
        }
        else 
            return checkVarNum(rest(cases), p);
    }
    return true;
}
...
export const typeofLit = (exp: LitExp, _tenv: TEnv, _p: Program): Result<TExp> =>
    isAtomicExp(exp.val) ? typeofExp(exp.val, _tenv, _p):
    makeOk(makeLitTExp());
...
export const typeofTypeCase = (exp: TypeCaseExp, tenv: TEnv, p: Program): Result<TExp> => {
    return bind(checkTypeCase(exp,p),()=>{
        ...
        for (let i = 0; i < exp.cases.length; i++) {
            let currCase = exp.cases[i];
            let rec = getRecordByName(currCase.typeName, p)
            let argsTyp = bind(rec,(rec)=> makeOk(rec.fields.map((f)=>f.te)))
            let argName = currCase.varDecls.map((v)=>v.var)
            let extTEnv = bind(argsTyp,(argsType)=> makeOk(makeExtendTEnv(argName, argsType, tenv)))
            let casesType = bind(extTEnv,(ext)=> typeofExps(currCase.body ,ext, p))
            if (isOk(casesType)){
