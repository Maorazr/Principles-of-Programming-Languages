...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) => 
        isEmpty(params) ? makeFailure("No arguments given"):
        !isEmpty(rest(params)) ? makeFailure("Too many arguments given"):
        parseGoodTraceExp(params[0]);
// HW3
const parseGoodTraceExp = (id: Sexp): Result<TraceExp> =>
    !isIdentifier(id) ? makeFailure("trace argument must be a valid identifier"):
    makeOk(makeTraceExp(makeVarRef(id)));
