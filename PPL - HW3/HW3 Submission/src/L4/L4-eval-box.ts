...
import { apply, map, repeat, zipWith } from "ramda";
...
const evalTraceExp = (exp: TraceExp, env: Env): Result<void> => {
    ...
    if (isOk(closure) && isClosure(closure.value) && isOk(binding)) {
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    ...
    proc.count ++;
    const value = applyClosure(proc.closure, args);
    console.log(proc.count);
    if (isOk(value)) {
        proc.count --;
        printPostTrace(value.value, proc.count);
        return value;
    }
    else {
        return makeFailure("failure");
...
export const evalTraceSequence = (seq: Exp[], env: Env): Result<Value> => {
